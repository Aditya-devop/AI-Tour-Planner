...
function CreateTripWizard() {
  ...
  const GetUserProfile = (tokeninfo) => {
    axios
      ...
      .then((res) => {
        localStorage.setItem("user", JSON.stringify(res.data));
        setOpenDialog(false);
        onGenerateTrip(); // proceed with generating the trip after login
      });
  };
  ...
  // Step-based validation in goNext
  const goNext = () => {
    // Step 1 checks
    if (currentStep === 1) {
      ...
      if (
        ...
        Number(formData.noOfDays) > 10
      ) {
        toast("Number of days must be between 1 and 10.");
        return;
      }
    }
  ...
  const onGenerateTrip = async () => {
    ...
    // Extra validation in case user tries to skip steps
    if (
      !formData?.Destination ||
      !formData?.Budget ||
      !formData?.Traveler ||
      ...
      toast("Please fill all details. Days must be 1–10.");
      return;
    }

    setLoading(true);
    toast("Please wait while we generate your itinerary…");
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 sm:px-10 md:px-16 lg:px-28 xl:px-32 mt-10 mb-10">
      {/* Heading / Intro */}
      <div className="text-center mb-10">
        <h2 className="font-bold text-3xl sm:text-4xl mb-3">
          <br />
          <br />
          Miles to Go, Memories to Make.
          <span className="ml-2 animate-pulse inline-block"></span>
        </h2>
        <p className="mt-3 text-gray-500 text-lg sm:text-xl">
          “Give us the scoop on your style—this trip is about to get personal!"
        </p>
      </div>
      ...
      <div className="flex items-center justify-center mb-10">
        {steps.map((step) => (
          <div key={step.id} className="flex items-center">
            ...
            {/* Step Label */}
            <div className="ml-2 mr-4 font-semibold text-sm text-gray-700">
              {step.label}
            </div>
            {/* Connector Line (except for last step) */}
            {step.id < steps.length && (
              <div
                className={`h-[2px] w-8 sm:w-16 transition-colors 
                ${currentStep > step.id ? "bg-blue-600" : "bg-gray-300"
                  }`}
              />
            )}
          ...
          {currentStep === 1 && (
            <div className="animate-fadeIn">
              <h2 className="text-xl font-semibold text-gray-800 mb-5">
                Step 1: Your Destination & Duration
              </h2>
              <div className="mb-8">
                <h3 className="text-lg mb-2">Ready? Let’s head out—pick your spot!</h3>
                <GooglePlacesAutocomplete
                  apiKey={import.meta.env.VITE_GOOGLE_PLACE_API_KEY}
                  selectProps={{
                    value: place,
                    ...
                    placeholder: `Enter your Escape`,
                    className:
                      "border border-gray-300 p-2 rounded-md focus:outline-none focus:border-blue-500 transition-all w-full shadow-sm",
                  }}
                />
              </div>
              <div>
                <h3 className="text-lg mb-2">
                  Your journey starts here! How long should the adventure last?
                </h3>
                <div className="flex flex-wrap gap-4">
                  <Input
                    type="number"
                    placeholder="days 1..10"
                    className="w-full md:w-1/2 focus:ring focus:ring-blue-200"
                    onChange={(e) =>
                  ...
                  />
                  <img src="calendar.svg" alt="" className="w-10 h-10" />
                </div>
              </div>
            </div>
          )}
          ...
          {currentStep === 2 && (
            <div className="animate-fadeIn">
              ...
              <p className="text-sm text-gray-500 mb-5">
                Choose your comfort zone for spending on this trip.
              </p>
              <p className="mb-3">Choose your comfort zone for spending on this trip.</p>
  ...
  {SelectBudgetOptions.map((item, index) => (
    <div
      ...
      className={`
        ...
        hover:-translate-y-1
        text-center
        ${formData?.Budget === item.title ? "shadow-xl border-blue-500" : "border-gray-200"}
      `}
    >
      {/* Slightly smaller icon/text to keep the card compact */}
      <div className="text-3xl mb-1">{item.icon}</div>
      <h2 className="font-bold text-base mt-1">{item.title}</h2>
      <p className="text-xs text-gray-500 mt-1">{item.desc}</p>
          ...
          {currentStep === 3 && (
            ...
            <p className="text-sm text-gray-500 mb-5">
              Select your companion(s) for this journey.
            </p>
        ...
        {/* Nav Buttons (Back / Next / Generate) */}
      ...
      <Dialog open={openDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogDescription>
              <img src="/logo3.svg" alt="company logo" className="mx-auto mb-5" />
              ...
              >
                <FcGoogle className="h-6 w-6" /> Sign in
              </Button>
            </DialogDescription>
          </DialogHeader>
        </DialogContent>
